...
Set_latest_new_version(){
	echo -e "请输入 要下载安装的 ocserv 版本 [ 格式: x.xx.x ，例如: 0.12.1 ]
...
check_new_ver(){
	...
	ocserv_ver=$(wget -qO- -t1 -T2 ftp://ftp.infradead.org/pub/ocserv/|grep File|grep -v '.sig'|awk '{print $(NF-2)}'|sed -r 's/.*tar\.xz\">ocserv-(.+)\.tar\.xz<\/a>.*/\1/'|grep -E '[0-9].[0-9][0-9].[0-9]'|sort -V | tail -1)
	if [[ -z ${ocserv_ver} ]]; then
		echo -e "${Error} ocserv 最新版本获取失败，请手动获取最新版本号[ ftp://ftp.infradead.org/pub/ocserv/ ]"
		stty erase '^H' && read -p "请输入版本号 [ 格式如 0.12.1 ] :" ocserv_ver
...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://github.com/hanyi9119/ocserv/blob/master/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate https://github.com/hanyi9119/ocserv/blob/master/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		wget --no-check-certificate -O "/etc/apt/sources.list" "https://github.com/hanyi9119/ocserv/blob/master/us.sources.list"
		apt-get update
